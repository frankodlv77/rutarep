...
function markerStyle(entrega) {
  if (!entrega)                      return { color: '#f59e0b', label: '●' }
  if (entrega.tipo === 'cancelado')  return { color: '#ef4444', label: '✕' }
  if (entrega.tipo === 'parcial')    return { color: '#8b5cf6', label: '½' }
  if (entrega.tipo === 'devolucion') return { color: '#3b82f6', label: '↩' }
...
export default function HoyMapView({ hoy, clientes, entregas }) {
  ...
  }, [hoy.join(','), JSON.stringify(entregas)])
